...
\begin{Exercise}[title=Variables] \label{Ex:Variables}
	...
	What is returned now when you {\tt print} {\tt x} and {\tt y}?}
...
\ExeText{As discussed in the lectures Python can be used as a calculator. You can input operations, assign values to variables, and store the results of operations for use in additional calculations.}
...
\begin{Exercise}[title=Overwriting Variables] \label{Ex:Overwriting_Variables}
    ...
    \Question{(*) What is the answer to $8 \div 2(2 + 2)$? Calculate this using Python. Is it what you expected? To learn more about this behaviour research the order and precedence of operators in Python.
    }
\end{Exercise}
...
\begin{Exercise}[title=Number Types]
    ...
    \textbf{Note:} This also occurs when you calculate $1 \div 2$. Although you are dividing two {\tt ints}, a {\tt float} is returned. 
    }
...
\begin{Exercise}[title=Boolean Operators]
    ...
    Once again Spyder helps us recognise that {\tt \color{Dandelion} True} and {\tt \color{Dandelion} False} are special {\tt keywords} by highlighting them using a different colour.}
...
\begin{Exercise}[title=Making a .py file]
    \Question{For larger programs you will want to save your work as a python file. 
    \begin{itemize}
        \item Create a new python file by clicking on ``File'' and then ``New File''.
        \item In your new file add the line {\tt NumberInFile=5}.
        \item Now click ``File'' and then ``Save''.
        \item You will be prompted to choose a file name and location. Name your file and save it somewhere appropriate.
