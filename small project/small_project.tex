...
\begin{Exercise}[title=Encryption and decryption]
	...
	\Question{All messages (either encrypted or decrypted) should be returned as \textbf{UPPER CASE} only.}
\end{Exercise}
...
\begin{Exercise}[title=Analysing messages]
	...
	\begin{itemize}
		...
		\item (Up to) The ten most common words sorted by their frequency in the format e.g., {\tt the~:~4} (i.e., "the" has been found 4 times)
		\item Minimum, maximum, and average word length;
		\item Most common letter;
	\end{itemize}}
	\Question{After the whole message has been encrypted or decrypted by your program, the program should print out the above statistics. The statistics should be based on the whole message; you do not need to print them for each line of the message.
\end{Exercise}
...
\begin{Exercise}[title=Automated decryption]
	\Question{Modify your program so that the user has an additional option for {\tt cipher mode}, {\tt auto-decryption}, in addition to the existing modes {\tt decryption} and {\tt encryption}.}
...
\begin{Exercise}[title=Extracting Information]
	\Question{Modify your program so the user now has an additional choice of importing a directory of files in addition to providing an input message and reading a message from a file.}
	...
	\Question{Decrypt all files in the folder 'sample_data'}
...
\end{itemize}}
\Question{You may try to improve on the automated encryption process the following ways:
\begin{itemize}
\item Add an alternative mode where the user can input {\tt random} instead of a {\tt rotation value}. The cipher will shift the text by a number of places equal to the random value generated.   
\item For each word in the message, you should select a rotation value at random, and then apply the rotation to the word (Update the decryption to decrypt word by word).
\end{itemize}}
